...
The server also never deletes a project it did not create:

- The post-failure cleanup that removes a `.gpr` and `.rep` only runs for an
  import, and importing into a directory you nominated is refused outright.
- `clean_cache(include_ghidra_projects=True)` only touches managed projects.

The one file the server will remove from your project is a stale `.lock` left
behind when a run of *ours* is killed mid-read. Ghidra drops its lock on a clean
exit but not on a kill, and every later run then refuses to open the project.
The program database is never touched.

## Relationship to managed project reuse

The server also reuses its *own* projects to avoid re-importing a binary on
every call — see [large-binary-decompile.md](large-binary-decompile.md). That
uses the same Ghidra mechanism (`-process`, read-only) and differs only in whose
project it is:

| | Managed reuse | Attach |
|---|---|---|
| Project | `<cache>/ghidra_projects`, content-hash named | Yours, named by you |
| Chosen by | An owner record the server wrote | You, via `ghidra_project` |
| On failure | Falls back to a fresh import | Reported — never silently re-imports |

That last row is the one worth knowing: if a managed project cannot be reused
the server just imports again and you get the same answer more slowly. An
attach names a specific project you asked to read, so a fresh import would
return a context with none of the work you came for. It fails loudly instead.
