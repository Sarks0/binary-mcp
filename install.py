...
def _activate_jython_extension(ghidra_dir: Path) -> None:
    ...
    Ghidra 12.1 unbundled Jython: the inert extension archive now ships under
    <GhidraDir>/Extensions/Ghidra/*Jython*.zip and must be extracted into the
    install-tree extensions dir (<GhidraDir>/Ghidra/Extensions/) for
    analyzeHeadless to load it -- the same effect as Front End -> Install
    Extensions. Earlier versions ship Jython built in. Failures here are
    non-fatal: the runtime gate in src/engines/static/ghidra/runner.py will
    still surface a clear error.
    """
    try:
        installed_dir = ghidra_dir / "Ghidra" / "Extensions"
        jython_dir = installed_dir / "Jython"
        if jython_dir.is_dir():
            print_info("Jython extension already activated, skipping")
            return
        # Bundled extension archives ship under <install>/Extensions/Ghidra/.
        # Fall back to the install-tree extensions dir in case a layout drops
        # the zip there instead.
        source_dirs = [ghidra_dir / "Extensions" / "Ghidra", installed_dir]
        matches: list[Path] = []
        for src in source_dirs:
            if src.is_dir():
                matches = list(src.glob("*Jython*.zip"))
                if matches:
                    break
        if not matches:
