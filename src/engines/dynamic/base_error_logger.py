"""
BaseErrorLogger - Shared error logging and tracking for dynamic analysis engines.

Stores detailed error information in structured JSON format for debugging and analysis.
Subclassed by engine-specific error loggers (X64DbgErrorLogger, WinDbgErrorLogger, etc.).
"""
...
class BaseErrorLogger:
    ...
    Subclasses must set:
    - ``_error_dir_name``: subdirectory under the cache folder (e.g. ``"x64dbg_errors"``)
    - ``_error_id_prefix``: short prefix for error IDs (e.g. ``"x64_"``)
    - ``_export_log_header``: header line for exported log files (e.g. ``"X64DBG ERROR LOG"``)
    """

    _error_dir_name: str = ""
    _error_id_prefix: str = ""
    _export_log_header: str = ""
    ...
    def __init__(self, error_dir: Path | None = None, max_errors: int = 500):
        ...
        logger.debug(f"{type(self).__name__} initialized: {self.error_dir}")
    ...
    def log_error(
        ...
        error_id = f"{self._error_id_prefix}{uuid.uuid4().hex[:16]}"
    ...
    def export_errors_log(self, output_file: Path) -> None:
        ...
        try:
            ...
            with open(output_file, "w") as f:
                f.write("=" * 80 + "\n")
                f.write(f"{self._export_log_header}\n")
