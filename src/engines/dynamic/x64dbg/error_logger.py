"""
X64DbgErrorLogger - Comprehensive error logging and tracking for x64dbg operations.

Stores detailed error information in structured JSON format for debugging and analysis.
Similar to Ghidra's debug logging but tailored for x64dbg's HTTP + Plugin architecture.
"""
...
class ErrorContext:
    ...
    operation: str  # Operation that failed (e.g., "step_into", "read_memory")
    ...
    binary_path: str | None = None  # Binary being debugged
...
class X64DbgErrorLogger:
    ...
    def __init__(self, error_dir: Path | None = None, max_errors: int = 500):
        ...
        logger.debug(f"X64DbgErrorLogger initialized: {self.error_dir}")
    ...
    def log_error(
        ...
        error_id = f"x64_{uuid.uuid4().hex[:16]}"
        ...
        logger.error(
            f"[{error_id}] {operation} failed: {error_type}: {str(error)}"
            for error_type in [type(error).__name__]
        )
        ...
        try:
            ...
            with open(output_file, "w") as f:
                ...
                for entry in manifest:
                    ...
                    f.write(f"Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(error.timestamp))}\n")
