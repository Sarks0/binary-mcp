...
def resolve_cached_binary(binary_ref: str) -> str:
    ...
    def accept(matches: list[dict]) -> str:
        ...
        if not os.path.isfile(path):
            ...
            raise BinaryResolutionError(
                f"'{ref}' was analyzed from {path}, but that file is no longer "
                f"there. The analysis cache is keyed on file contents, so the "
                f"binary has to be readable somewhere -- restore it, or pass "
                f"the path where it lives now."
            )
        return path
    ...
    candidates = "\n".join(f"  - {cached_path(e)}" for e in by_name[:10])
    raise BinaryResolutionError(
        f"'{ref}' matches {len(by_name)} analyzed binaries. Pass a full path:\n"
        f"{candidates}"
    )
...
def register_dynamic_tools(app: FastMCP, session_manager: UnifiedSessionManager | None = None) -> None:
    ...
    def x64dbg_get_modules() -> str:
        ...
        try:
            ...
            for mod in modules:
                ...
                result.append(f"  Base: 0x{mod['base']:X}")
                result.append(f"  Size: 0x{mod['size']:X}")
                if mod["path"] and mod["path"] != mod["display_name"]:
                    result.append(f"  Path: {mod['path']}")
    ...
    # Register one tool per group, appending the generated operation
    # catalog so the documented arguments always match the real ones.
    group_tools = {
        "session": x64dbg_session,
        "state": x64dbg_state,
        "execution": x64dbg_execution,
        "breakpoint": x64dbg_breakpoint,
        "memory": x64dbg_memory,
        "context": x64dbg_context,
        "thread": x64dbg_thread,
        "module": x64dbg_module,
        "disasm": x64dbg_disasm,
        "search": x64dbg_search,
        "analyze": x64dbg_analyze,
        "types": x64dbg_types,
        "trace": x64dbg_trace,
        "annotate": x64dbg_annotate,
        "symbols": x64dbg_symbols,
        "hooks": x64dbg_hooks,
    }

    for _group, _fn in group_tools.items():
        _fn.__doc__ = inspect.cleandoc(_fn.__doc__ or "") + build_op_catalog(ops[_group])
        app.tool()(_fn)
    ...
    logger.info(
        f"Registered {len(group_tools)} grouped dynamic analysis tools "
