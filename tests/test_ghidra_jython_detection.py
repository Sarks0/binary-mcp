...
def _no_real_user_extensions(monkeypatch):
    """Neutralise the per-user settings Jython scan by default.

    Without this, ``_jython_available`` would walk the developer's real home
    dir, and the negative tests (extension absent -> must raise) would flip to
    green on any machine that happens to have Jython installed system-wide.
    Tests that exercise the user-settings path re-override this seam.
    """
    monkeypatch.setattr(
...
def test_user_settings_installed_jython_is_honored(tmp_path, monkeypatch):
    """Jython installed via Front End -> Install Extensions lands in the
    per-user settings dir, not the install tree. The gate must honor it
    rather than false-positive on a correctly-configured machine."""
    install = _stub_ghidra_install(tmp_path, version="12.1")
    settings_root = tmp_path / "userconfig" / "ghidra"
    jdir = settings_root / "ghidra_12.1_PUBLIC" / "Extensions" / "Jython" / "lib"
    jdir.mkdir(parents=True)
    (jdir / "jython-standalone.jar").write_bytes(b"PK\x03\x04")

    # Re-override the autouse seam to point at our fake settings root.
    monkeypatch.setattr(
        GhidraRunner,
        "_user_settings_extension_roots",
        lambda self: [settings_root],
    )
    runner = GhidraRunner(ghidra_path=str(install))

    runner.ensure_jython_available()  # must not raise
    assert runner._jython_available() is True
    assert runner.diagnose()["jython_available"] is True


def test_user_settings_without_jython_still_raises(tmp_path, monkeypatch):
    """An empty per-user settings root must not satisfy the gate."""
    install = _stub_ghidra_install(tmp_path, version="12.1")
    settings_root = tmp_path / "userconfig" / "ghidra"
